...
struct ReviewSheet: View {
    ...
    @State private var batchReview: String?
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let result {
                ...
                if let batchReview {
                    AISummaryBanner(text: batchReview) { self.batchReview = nil }
                        .padding(.horizontal)
                        .padding(.bottom, 10)
                }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 10) {
                ...
                // Deliberately disabled rather than the usual "tap to add a key" flow used
                // elsewhere — this is the last screen before something gets deleted, so it should
                // only ever run when AI Assist is genuinely configured, not offer to set it up
                // mid-review.
                .disabled(isReviewingBatch || !AIAssistService.isAvailable || itemsToDelete.isEmpty)
                .help(AIAssistService.isAvailable
                      ? "Ask AI to summarize this selection and flag anything worth double-checking before you confirm"
    ...
    private func reviewBatch() async {
        isReviewingBatch = true
        defer { isReviewingBatch = false }
        do {
            batchReview = try await AIAssistService.reviewBeforeDeleting(items: itemsToDelete)
        } catch {
            batchReview = error.localizedDescription
        }
    }
