    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if viewModel.isLoadingRoot {
                ...
                ScrollView {
                    // `LazyVStack`, not `VStack` — with a plain `VStack`, SwiftUI laid out every
                    // row in the entire expanded tree up front, whether it was on screen or not.
                    // Expanding a large folder (or several) meant instantiating and animating
                    // potentially thousands of off-screen rows at once, which is what made both
                    // expanding *and* collapsing feel like they'd locked up.
...
private struct RootChildrenView: View {
    ...
    private var unreadableBanner: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Permission was denied. This usually means Full Disk Access is needed for MacMemMan to see this location.")
                .font(.caption)
                .foregroundStyle(.secondary)
            HStack {
                Button("Grant Access…") {
...
private struct TreeRowView: View {
    ...
    // Computed once per row body evaluation instead of three separate times (icon, badge, help
    // text each used to call `node.asScanItem` — which rebuilds a `ScanItem` and re-runs the
    // safety assessor — independently). Trivial for one row; multiplied across a large expanded
    // tree it was real, repeated work for no reason.
    private var safety: SafetyAssessment { node.asScanItem.safety }
    ...
    private var rowContent: some View {
        HStack(spacing: 8) {
            ...
            if node.isDirectory {
                Button {
                    ...
                    // The glyph itself is still small, but the *tappable* area is a full 22x22pt
                    // square around it (`.contentShape` extends hit-testing beyond the visible
                    // pixels) — the old version only registered clicks on the few pixels the
                    // chevron glyph actually covered, which is exactly what made it feel like you
                    // had to click precisely.
            ...
            Image(systemName: node.isDirectory ? "folder.fill" : (safety.level == .personal ? "photo.fill" : "doc.fill"))
            ...
            if !node.isDirectory {
                SafetyBadge(level: safety.level, showLabel: false)
            }
        ...
        .contentShape(Rectangle())
        .help(node.isDirectory ? node.url.path : safety.reason)
    }
}
