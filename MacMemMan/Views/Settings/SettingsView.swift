        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            Toggle(isOn: $autoCleanup.isEnabled) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Runs while MacMemMan is open — it never launches itself in the background. Findings always need your approval before anything is deleted.")
                        .font(.caption)
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            Toggle(isOn: $appSettings.menuBarOnlyMode) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Menu bar only")
                        .font(.system(.body, design: .rounded))
                    Text("Hides the Dock icon — MacMemMan lives purely in the menu bar (\(Image(systemName: "wind"))). Reopen the window anytime from the menu bar dropdown's \"Open MacMemMan\".")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
