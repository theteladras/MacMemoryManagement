...
enum FileSystemScanner {
    ...
    static func immediateChildren(of directory: URL, includeHidden: Bool = false) -> DirectoryListing {
        ...
        // Capped, not fully unbounded: a folder like `~/Library` can easily have 100+
        // subdirectories, and firing off a `du` subprocess for every single one at once causes
        // real disk I/O contention — each individual `du` ends up *slower* under that contention,
        // which is what made expanding a large folder feel like it hung rather than sped up.
        // `DispatchQueue.concurrentPerform` still schedules every iteration, but the semaphore
        // keeps only a handful of `du` processes actually running at a time.
        let results = ThreadSafeBox<[String: (Int64, SizeStatus)]>([:])
        let concurrentDuLimit = DispatchSemaphore(value: 8)
        DispatchQueue.concurrentPerform(iterations: candidates.count) { index in
            let candidate = candidates[index]
            let result: (Int64, SizeStatus)
            if candidate.isDir {
                concurrentDuLimit.wait()
                result = sizeAndStatus(of: candidate.url)
                concurrentDuLimit.signal()
            } else {
                result = (candidate.fileSize, .ok)
            }
